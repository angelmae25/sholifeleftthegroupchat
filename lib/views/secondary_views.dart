        ...
        return GridView.builder(
          ...
          itemBuilder: (_, i) {
            ...
            return Container(
              ...
              child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                _OrgLogoAvatar(
                  logoUrl: isOrg ? club.logoUrl : null,  // only orgs have logos
                  icon: club.icon,
                  color: club.color,
                  radius: 32,
                ),
      ...
      builder: (_, scrollCtrl) => Container(
        ...
        child: Column(children: [
          ...
          Padding(
            ...
            child: Row(children: [
              _OrgLogoAvatar(
                // Use club.logoUrl immediately (already loaded in grid),
                // then upgrade to the API value once fetched.
                // .toString() avoids the silent Object->String? cast failure.
                logoUrl: _loading
                    ? widget.club.logoUrl
                    : (_detail['logoUrl']?.toString()?.isNotEmpty == true
                    ? _detail['logoUrl']!.toString()
                    : widget.club.logoUrl),
                icon: widget.club.icon,
                color: color,
                radius: 26,
              ),
...
// ─────────────────────────────────────────────────────────────────────────────
// ORG LOGO AVATAR — shows real logo, falls back to icon
// ─────────────────────────────────────────────────────────────────────────────
class _OrgLogoAvatar extends StatelessWidget {
  final String? logoUrl;
  final IconData icon;
  final Color color;
  final double radius;

  const _OrgLogoAvatar({
...
// ─────────────────────────────────────────────────────────────────────────────
// REPORT LOST / FOUND VIEW
// Paste this at the very bottom of secondary_views.dart
// ─────────────────────────────────────────────────────────────────────────────
      ...
      body: SingleChildScrollView(
        ...
        child: Form(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            // ── Status toggle ─────────────────────────────────────────────
            _RLFLabel('Report Type *', textMain),
            const SizedBox(height: 8),
            Container(
              decoration: BoxDecoration(
                  color: inputFill,
              ...
              child: Row(children: [
                _RLFStatusBtn('Lost',  'lost',  _status,
                    const Color(0xFFB71C1C),
                        (v) => setState(() => _status = v)),
                _RLFStatusBtn('Found', 'found', _status,
                    const Color(0xFF2E7D32),
                        (v) => setState(() => _status = v)),
              ]),
            ),
            ...
            // ── Photo ─────────────────────────────────────────────────────
            _RLFLabel('Photo (Optional)', textMain),
            const SizedBox(height: 8),
            GestureDetector(
            ...
            // ── Title ─────────────────────────────────────────────────────
            _RLFLabel('Item Name *', textMain),
            const SizedBox(height: 8),
            TextFormField(
              controller: _titleCtrl,
              textCapitalization: TextCapitalization.words,
              style: TextStyle(color: textMain),
              decoration: _rlfInputDeco(
            ...
            // ── Description ───────────────────────────────────────────────
            _RLFLabel('Description *', textMain),
            const SizedBox(height: 8),
            TextFormField(
            ...
            // ── Date ──────────────────────────────────────────────────────
            _RLFLabel(isLost ? 'Date Lost *' : 'Date Found *', textMain),
            const SizedBox(height: 8),
            GestureDetector(
              onTap: _pickDate,
              child: Container(
...
class _RLFStatusBtn extends StatelessWidget {
...
// ── Helpers ───────────────────────────────────────────────────────────────────
Widget _RLFLabel(String text, Color color) => Text(text,
    style: TextStyle(
        fontSize: 13, fontWeight: FontWeight.w700, color: color));

InputDecoration _rlfInputDeco(
