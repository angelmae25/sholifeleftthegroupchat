// FILE PATH: lib/views/secondary_views.dart
// FIXES APPLIED:
//   1. _ItemCard — image tap opens full-screen preview
//   2. _chatSeller — async, awaits socket, properly navigates
//   3. _contactReporter — same fix as _chatSeller
//   4. LeaderboardView — removed Container that broke Expanded height
//   5. _PodiumCol — added text overflow to prevent column overflow
//   6. ProfileView — UserService.fetchProfile now calls correct endpoint
//   7. LeaderboardView list row — fixed Row overflow by 48px (Flexible + ellipsis)
...
class _MarketplaceViewState extends State<MarketplaceView> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) =>
        context.read<MarketplaceController>().loadItems());
  }
    ...
    return AppScaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () => Navigator.push(
            context, MaterialPageRoute(builder: (_) => SellItemView())),
...
// ─────────────────────────────────────────────────────────────────────────────
// ITEM CARD  (FIX 1: image tap preview  |  FIX 2: async chat DM)
// ─────────────────────────────────────────────────────────────────────────────
class _ItemCard extends StatelessWidget {
  final MarketplaceItemModel item;
  const _ItemCard({required this.item});

  Future<void> _chatSeller(BuildContext context, String sellerId) async {
    ...
    await ChatSocketService.instance.startDm(sellerId);
  }
  ...
  void _openImage(BuildContext context) {
    ...
    showDialog(
      ...
      builder: (_) => _FullImageViewer(imageUrl: url, title: item.name),
    );
  }
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        GestureDetector(
          ...
          child: ClipRRect(
            ...
            child: Stack(
              children: [
                _ItemImage(imageUrl: item.imageUrl, height: 110),
        ...
        Padding(
          ...
          child: Column(
              ...
              children: [
                ...
                Text(item.condition,
                    style: TextStyle(
                        color: AppTheme.textSub(context), fontSize: 11)),
...
// ─────────────────────────────────────────────────────────────────────────────
// FULL IMAGE VIEWER  (reusable for both Marketplace + Lost&Found)
// ─────────────────────────────────────────────────────────────────────────────
class _FullImageViewer extends StatelessWidget {
  final String imageUrl;
  final String title;
  const _FullImageViewer({required this.imageUrl, required this.title});
  ...
  Widget build(BuildContext context) {
    return Dialog(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        const Text('Pinch to zoom  •  Tap outside to close',
...
class _LostFoundViewState extends State<LostFoundView> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback(
            (_) => context.read<LostFoundController>().loadItems());
  }
  ...
  Widget build(BuildContext context) {
    ...
    return AppScaffold(
      title: 'Lost & Found',
      currentRoute: '/lost-found',
      body: Consumer<LostFoundController>(
        builder: (_, ctrl, __) => Container(
          color: pageClr,
          child: Column(children: [
            Container(
              ...
              child: Container(
                ...
                child: Row(children: [
                  _ToggleBtn(
                      'Lost Items',
                      ctrl.activeTab == LostFoundStatus.lost,
                          () => ctrl.setTab(LostFoundStatus.lost)),
                  _ToggleBtn(
                      'Found Items',
                      ctrl.activeTab == LostFoundStatus.found,
                          () => ctrl.setTab(LostFoundStatus.found)),
                ]),
              ),
            ),
            if (ctrl.isLoading)
            ...
            if (!ctrl.isLoading)
              Expanded(
                child: ListView(
                    ...
                    children: ctrl.filteredItems
                        .map((item) => _LostFoundCard(item: item))
                        .toList()),
              ),
          ]),
        ),
      ...
      Consumer<LostFoundController>(builder: (_, ctrl, __) {
        final isLost = ctrl.activeTab == LostFoundStatus.lost;
        return FloatingActionButton.extended(
          onPressed: () => Navigator.push(
              context,
              MaterialPageRoute(
                  builder: (_) => ReportLostFoundView(
                      initialStatus: isLost ? 'lost' : 'found'))),
...
// ─────────────────────────────────────────────────────────────────────────────
// LOST & FOUND CARD  (FIX 3: async contact reporter)
// ─────────────────────────────────────────────────────────────────────────────
class _LostFoundCard extends StatelessWidget {
  ...
  void _openImage(BuildContext context, Color color) {
    ...
    showDialog(
      ...
      barrierColor: Colors.black87,
      builder: (_) =>
          _FullImageViewer(imageUrl: item.imageUrl!, title: item.title),
    );
  }

  Future<void> _contactReporter(
      BuildContext context, String reporterId) async {
    final myId = context.read<AuthController>().user?.id ?? '';
    if (reporterId == myId) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('This is your own report.'),
          behavior: SnackBarBehavior.floating));
      return;
    }
    ...
    await ChatSocketService.instance.startDm(reporterId);
  }
  ...
  Widget build(BuildContext context) {
    ...
    final color =
    isLost ? const Color(0xFFB71C1C) : const Color(0xFF2E7D32);
    final cardClr = AppTheme.cardColor(context);
    final hasImage = item.imageUrl != null && item.imageUrl!.isNotEmpty;
                  ...
                  children: [
                    ...
                    Row(children: [
                      Icon(Icons.location_on_outlined,
                          size: 13, color: AppTheme.textSub(context)),
              ...
              child: Column(
                  ...
                  children: [
                    ...
                    Text('No conversations yet.',
                        style: TextStyle(
                            color: AppTheme.textSub(context), fontSize: 14)),
                    const SizedBox(height: 6),
                    Text(
                        'Tap "Chat Seller" on a marketplace item\nor "Contact" on a lost & found report.',
                        textAlign: TextAlign.center,
                        style: TextStyle(
                            color: AppTheme.textSub(context), fontSize: 12)),
                  ]),
            );
          }
    ...
    child: ListTile(
      ...
      leading: CircleAvatar(
        ...
        child: Icon(
            ...
            color:
            chat.isGroup ? AppTheme.accent : AppTheme.primary,
            size: 26),
      ),
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      borderRadius: BorderRadius.only(
                        ...
                        bottomLeft:
                        Radius.circular(msg.isMine ? 16 : 4),
                        bottomRight:
                        Radius.circular(msg.isMine ? 4 : 16),
                      ),
                      boxShadow: [
                        BoxShadow(
...
class _ClubsViewState extends State<ClubsView> {
  ...
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback(
            (_) => context.read<ClubsController>().loadClubs());
  }

  void _showOrgDetail(BuildContext context, ClubModel club) {
    final rawId = club.id.startsWith('org_')
        ? club.id.replaceFirst('org_', '')
        : club.id;
              ...
              child: Column(
                  ...
                  children: [
                    Container(
                      ...
                      height: 64,
                      decoration: BoxDecoration(
                          color: club.color.withValues(alpha: 0.12),
                          shape: BoxShape.circle),
                    ...
                    if (isOrg)
                      GestureDetector(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            color: club.color.withValues(alpha: 0.1),
                            borderRadius: BorderRadius.circular(20),
                            border: Border.all(
                                color: club.color.withValues(alpha: 0.3)),
                          ),
  ...
  Future<void> _fetchDetail() async {
    ...
    try {
      final res = await http
          .get(Uri.parse('$_base/organizations/${widget.orgId}'))
      ...
      if (res.statusCode == 200) {
        ...
        setState(() {
          _error = 'Could not load details (${res.statusCode}).';
          _loading = false;
        });
      }
    } catch (e) {
        ...
        child: Column(children: [
          ...
          Expanded(
            child: _loading
              ...
              children: [
                _InfoRow(Icons.person_outline, 'Adviser',
                ...
                _InfoRow(
                    ...
                    _detail['yearFounded'] != null
                        ...
                        : '—'),
                if ((_detail['description'] as String?)
                    ?.isNotEmpty ==
                    true) ...[
...
class _LeaderboardViewState extends State<LeaderboardView> {
  ...
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback(
            (_) => context.read<LeaderboardController>().loadLeaderboard());
  }
        ...
        return Column(children: [
          ...
          Container(
            ...
            child: Row(
              ...
              crossAxisAlignment: CrossAxisAlignment.end,
              children: [
                if (ctrl.topThree.length > 1)
                  Flexible(
                      child: _PodiumCol(
                          entry: ctrl.topThree[1], height: 80)),
                const SizedBox(width: 12),
                if (ctrl.topThree.isNotEmpty)
                  Flexible(
                      child: _PodiumCol(
                          entry: ctrl.topThree[0], height: 100)),
                const SizedBox(width: 12),
                if (ctrl.topThree.length > 2)
                  Flexible(
                      child: _PodiumCol(
                          entry: ctrl.topThree[2], height: 64)),
              ],
            ),
          ),
          // Points hint banner
          Container(
            ...
            child: Row(children: [
              ...
              Expanded(
                  child: Text(
                      'Attend events to earn points and climb the leaderboard!  +10 pts per event',
                      style: TextStyle(
                          color: AppTheme.textSub(context),
                          fontSize: 11,
              ...
              itemBuilder: (_, i) {
                ...
                return Container(
                  ...
                  decoration: BoxDecoration(
                      color: AppTheme.cardColor(context),
                      borderRadius: BorderRadius.circular(12)),
                  // FIX 7: Row was overflowing 48px on the right.
                  // Expanded child column now has ellipsis on both text
                  // widgets, and the trailing pts label is wrapped in
                  // Flexible so it can compress instead of overflowing.
                  child: Row(children: [
                    Container(
                      width: 32,
                      height: 32,
                    ...
                    const SizedBox(width: 12),
                    _LeaderboardAvatar(
                        avatarUrl: e.avatarUrl, radius: 20),
                    const SizedBox(width: 12),
                    Expanded(
                        child: Column(
                            crossAxisAlignment:
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 2),
        _LeaderboardAvatar(avatarUrl: entry.avatarUrl, radius: 22),
        const SizedBox(height: 2),
        SizedBox(
          width: 70,
          child: Text(
        ...
        return SingleChildScrollView(
            child: Column(children: [
                ...
                child: Column(children: [
                  GestureDetector(
                    onTap: () => Navigator.push(
                        ...
                        MaterialPageRoute(
                            builder: (_) =>
                                EditProfileView(user: user))).then((_) {
                    ...
                    child: Stack(children: [
                      _ProfileAvatar(
                          avatarUrl: user.avatarUrl, radius: 48),
                      Positioned(
                        bottom: 0,
                        right: 0,
                        child: Container(
              ...
              _InfoSection('Personal Information', [
                ...
                _InfoRow(Icons.calendar_today_outlined, 'Year Level',
                    user.yearLevel),
              ]),
              const SizedBox(height: 8),
          ...
          builder: (_, ctrl, __) => ListView(
            ...
            children: [
              _SettingsGroup('Notifications', [
                _ToggleRow('Push Notifications', 'Receive app notifications',
                    Icons.notifications_outlined, ctrl.pushNotifications,
                    ctrl.setPushNotifications),
                _ToggleRow('Email Alerts', 'Get updates via email',
                    Icons.email_outlined, ctrl.emailAlerts,
                    ctrl.setEmailAlerts),
              ]),
              const SizedBox(height: 12),
              _SettingsGroup('Appearance', [
                _ToggleRow('Dark Mode', 'Switch to dark theme',
                    Icons.dark_mode_outlined, ctrl.darkMode,
                    ctrl.setDarkMode),
              ]),
              const SizedBox(height: 12),
              _SettingsGroup('Privacy', [
                _ToggleRow('Location Access', 'Allow location for events',
                    Icons.location_on_outlined, ctrl.locationAccess,
                    ctrl.setLocationAccess),
              ]),
              const SizedBox(height: 12),
              _SettingsGroup('Account', [
                ...
                _TapRow('Report a Problem', Icons.flag_outlined,
...
void _showChangePasswordDialog(BuildContext context) {
  ...
  showDialog(
    ...
    builder: (ctx) =>
        StatefulBuilder(builder: (ctx, setState) => AlertDialog(
          shape:
            ...
            ElevatedButton(
              ...
              onPressed: isSubmitting
                ...
                final ok = await context
                    .read<AuthController>()
                    .changePassword(
                    current: current, newPassword: newPw);
                if (!context.mounted) return;
                if (ok) {
                  Navigator.pop(ctx);
              ...
              onPressed: isSubmitting
                ...
                try {
                  ...
                  final res = await http.post(
                    Uri.parse(
                        'http://192.168.1.11:5000/api/mobile/reports/'),
                    headers: {
  ...
  Widget build(BuildContext context) {
    ...
    return CircleAvatar(
      ...
      child: Icon(Icons.person,
          size: radius * 1.1, color: AppTheme.primaryDark),
    );
  }
}
